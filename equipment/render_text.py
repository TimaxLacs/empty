...
def render() -> str:
    ...
    out.append("════════════════════════════════════════")
    out.append("СПЕЦИАЛИЗАЦИИ")
    out.append("════════════════════════════════════════")
    out.append("")
    by_role: dict[str, list[str]] = defaultdict(list)
    for person in DATA["people"]:
        by_role[person["role"]].append(person["name"])
    role_order = ["Стрелок", "Сапёр", "Гранатомётчик", "Марксман"]
    for role in role_order:
        names = by_role.get(role, [])
        out.append(f"{role}: {', '.join(names) if names else 'никого'}")
    out.append("Пулемётчик: никого")
    out.append("Медик: никого")
    out.append("Снайпер: никого")
    out.append("")
    out.append("Стрелок — Геккон, Шуга, Рыжий, Шершень, Матрос")
    out.append("Отдельной спецэкипировки нет. Сдвоенный подсумок и сбросник идут по базовому списку.")
    out.append("Сдвоенный подсумок есть: Шершень")
    out.append("Сдвоенный подсумок нет: Геккон, Шуга, Рыжий, Матрос")
    out.append("Сбросник есть: Шуга, Шершень, Матрос")
    out.append("Сбросник нет: Геккон, Рыжий")
    out.append("")
    out.append("Сапёр — Горизонт, Овод")
    out.append("Воротник — есть: Горизонт, Овод")
    out.append("Наплечники — есть: Горизонт, Овод")
    out.append("Защита предплечья — есть: Горизонт, Овод")
    out.append("Набедренные платформы — есть: Горизонт, Овод")
    out.append("Защита голени — есть: Горизонт, Овод")
    out.append("Подсумок для МПЛ — есть: Горизонт, Овод")
    out.append("МПЛ — есть: Горизонт, Овод")
    out.append("Визор — есть: Горизонт, Овод")
    out.append("Пятиточечник — есть: Горизонт, Овод")
    out.append("Чехол на лопату — нет: Овод")
    out.append("")
    out.append("Гранатомётчик — Велес")
    out.append("Подсумок для выстрелов гранатомёта — нет: Велес")
    out.append("Напашный подсумок / напашник — нет: Велес")
    out.append("")
    out.append("Марксман — Арчи")
    out.append("Коврик для стрельбы — нет: Арчи")
    out.append("Подсумок для магазинов винтовки — не сказано: Арчи")
    out.append("Пятиточечник — не сказано: Арчи")
    out.append("")
